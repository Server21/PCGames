document.addEventListener("DOMContentLoaded", function() {
  ...
  // Mostra il nome dell'utente e il numero di utenti online (simulato)
  document.getElementById("userDisplay").innerText = user;
  document.getElementById("onlineUsers").innerText = "Utenti online: " + (parseInt(localStorage.getItem("onlineCount")) || 1);

  // Carica le informazioni sull'abbonamento da accounts.json
    ...
    .then(accounts => {
      if (accounts[user]) {
        ...
        if (typeof accountData === "object" && accountData.expires) {
          ...
          document.getElementById("subscriptionInfo").innerText = "Abbonamento scade tra " + diffDays + " giorni.";
        } else {
          document.getElementById("subscriptionInfo").innerText = "Abbonamento a vita.";
        }
      }
    })
  ...
  function displayGames(games) {
    ...
    games.forEach(game => {
      ...
      // Pulsante per il download (apre l'iframe nella pagina stessa)
  ...
  // Funzionalità chat: massimo 2 messaggi al giorno
